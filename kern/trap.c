...
{
	//cprintf("tf -> tf_trapno:%d, rcr3():%08x\n",tf -> tf_trapno,rcr3());
...
trap(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 3) {
		...
		//原来内核的页目录地址被扔了 为啥没有关系呢？
		//为什么现在在内核 但是cr3 是用户态的 然后能输出内核态地址的东西？
		//int 中断 和 普通中断有何不同?
		//为什么如果开放page fault给用户 把DPL设成3？ 会有什么后果？
		//kernel的cs存放在哪里？ 
		//tf_cs是何时压入? 
		//tr GDT TSS  的关系
		//我们通过lcr3来设置cr3的值倒是侯硬件就自己读了，硬件从哪读段选择子呢？	
		//如果段基地址不是0会怎么样，我们写的代码到底是段寻址中的offset 还是 段后的虚拟地址
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if( curenv-> env_pgfault_upcall != NULL)
	{
		...
		//the trap is from the user exception handler
		if(tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP)
			utf = (struct UTrapframe *)( tf->tf_esp - sizeof (struct UTrapframe) - 4 );
		else
			utf = (struct UTrapframe *)( UXSTACKTOP - sizeof (struct UTrapframe) );
		//check the adress at exception stack
		user_mem_assert( curenv, (void*)utf, sizeof (struct UTrapframe) , PTE_U|PTE_W);
	
		//push the struct UTrapframe into the utf stack below the UXSTACKTOP
		...
		//set the eip to run the handler and esp point to exception stack
