	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct PageInfo *pp;
	pte_t *tmpPte;
	pp = page_lookup(srcenv->env_pgdir, srcva, &tmpPte);
	if(pp == NULL || ( ((perm & PTE_W) > 0) && ((*tmpPte & PTE_W) == 0))) return -E_INVAL;
...
sys_exec(envid_t envid)
{
	...
	struct Env *src_env;
	//-E_BAD_ENV if environment envid doesn't currently exist.
	if((r=envid2env(envid, &src_env, 0)) < 0)
	{
		return r;
	}
	//step1:copy all info
	envid_t tmp_envid = curenv->env_id;
	memmove(curenv,  src_env, sizeof(struct Env));
	curenv->env_status = ENV_RUNNABLE;
	curenv->env_id = tmp_envid;

	//step2:destory src_env
	env_free_simple(src_env);
	return 0;
}
